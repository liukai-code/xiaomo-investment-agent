package com.xiaomo.agent.workflow.service;
...
import com.xiaomo.agent.workflow.agent.WorkflowAgentFactory;
import com.xiaomo.agent.workflow.config.RiskOverrideProperties;
import com.xiaomo.agent.workflow.config.WorkflowProperties;
import com.xiaomo.agent.workflow.engine.WorkflowEngine;
import com.xiaomo.agent.workflow.engine.WorkflowGraph;
import com.xiaomo.agent.workflow.event.WorkflowEvent;
import com.xiaomo.agent.workflow.node.*;
...
import com.xiaomo.agent.user.service.FreeQuotaService;
...
public class DeepAnalysisWorkflow {
    ...
    private final ConcurrentHashMap<Long, com.xiaomo.agent.workflow.state.WorkflowState> runningStates = new ConcurrentHashMap<>();
    ...
    public void cancelAnalysis(Long analysisId) {
        ...
        com.xiaomo.agent.workflow.state.WorkflowState state = runningStates.get(analysisId);
        if (state != null) {
            state.signalCancel();
            // 取消时也要扣减已消耗的免费额度
