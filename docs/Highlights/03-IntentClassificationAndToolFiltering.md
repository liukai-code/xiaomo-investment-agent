...
- 掌握 `RuleBasedIntentClassifier` 的 4 级优先匹配机制和 9 种意图类型
...
1. **毫秒级分类**：不依赖 LLM 调用，纯规则匹配，延迟 < 5ms
2. **高特异性优先**：越具体的意图越先匹配，避免被宽泛意图"吞掉"
3. **业务意图与执行方式分离**："分析茅台"和"深度分析茅台"的业务意图相同（都是个股分析），但执行方式不同（单步 vs 多步规划）
4. **可扩展**：新增意图类型只需添加关键词和工具映射，不改主流程
...
用户消息进入后，先用关键词规则判断"用户想做什么"（意图分类），再根据意图从 44 个工具中筛选出相关的子集（工具过滤），只把这把"精简工具箱"交给 LLM。
...
| 组件 | 文件路径 | 职责 |
|------|---------|------|
| `RuleBasedIntentClassifier` | `agent/intent/RuleBasedIntentClassifier.java` | 4 级优先关键词匹配，输出 `IntentResult` |
...
`classifyBusinessIntent()` 按固定优先级顺序匹配，**命中即返回**：
...
### 4.5 执行模式判断：TaskPlanner.determineExecutionMode()
...
```mermaid
flowchart LR
    IC[意图分类器] -->|IntentResult| Agent[AgentLoopImpl]
...
| 文件 | 路径 | 关键方法 |
|------|------|---------|
| RuleBasedIntentClassifier | `src/main/java/com/xiaomo/agent/agent/intent/RuleBasedIntentClassifier.java` | `classify()`, `classifyBusinessIntent()`, `tryResolveStock()` |
...
| 测试 | `src/test/java/com/xiaomo/agent/agent/intent/RuleBasedIntentClassifierTest.java` | 12 个测试组，覆盖所有意图类型 |
